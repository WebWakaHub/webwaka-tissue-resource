/**
 * ResourceTissue — Unit Tests
 * Tissue ID: TIS-RESOURCE-v0.1.0
 * Test Hash: 771cc120
 */

import { ResourceTissue } from '../src/entity';
import { NIGERIA_FIRST_CONFIG, TISSUE_ID, COMPOSED_CELLS } from '../src/types';

describe('ResourceTissue', () => {
  let tissue: ResourceTissue;

  beforeEach(() => {
    tissue = new ResourceTissue();
  });

  describe('Identity', () => {
    it('should have correct tissue ID', () => {
      expect(TISSUE_ID).toBe('TIS-RESOURCE-v0.1.0');
    });

    it('should compose correct cells', () => {
      expect(COMPOSED_CELLS).toEqual(['CEL-RESOURCEPOOL', 'CEL-RESOURCEALLOC', 'CEL-RESOURCEMON']);
    });
  });
  ...
  describe('Coordination', () => {
    it('should coordinate requests with Nigeria-first defaults', async () => {
      ...
      expect(result.requestId).toBe('test-771cc120-001');
  ...
  describe('Offline First (NON-NEGOTIABLE)', () => {
    it('should queue requests when offline', async () => {
      const request = {
        requestId: 'offline-771cc120-001',
        sourceCell: 'CEL-RESOURCEPOOL',
        targetCells: ['CEL-RESOURCEPOOL'],
    ...
    it('should report queue depth in health check', async () => {
      ...
      expect(health.tissueId).toBe('TIS-RESOURCE-v0.1.0');
